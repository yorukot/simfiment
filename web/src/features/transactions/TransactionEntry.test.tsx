...
describe("TransactionEntry", () => {
  it("keeps save disabled for zero, then sends one transaction with an empty title", async () => {
    ...
    const nextAmount = screen.getByRole("textbox", { name: /金額/ });
    expect(nextAmount).toHaveValue("");
    ...
    const allPosts = fetchMock.mock.calls.filter(
      (call) => String(call[0]) === "/api/v1/transactions",
    );
