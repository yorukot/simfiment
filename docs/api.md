...
API base path 為 `/api/v1`。成功 JSON 使用 `{ "data": ... }`，錯誤使用 `{ "error": { "code", "message", "fields", "requestId" } }`；CSV 下載端點則直接回傳檔案。除了 meta、setup、login 與 health 外，所有端點都需要 Session cookie；所有 authenticated mutation 另外需要同源 `Origin` 與 `X-CSRF-Token`。
