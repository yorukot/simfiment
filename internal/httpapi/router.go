...
func New(svc *service.Service, db *sql.DB, cfg platform.Config, logger *slog.Logger, version string, static http.Handler) http.Handler {
	...
	mux.HandleFunc("GET /api/v1/budgets", a.protected(false, a.getBudgets))
	mux.HandleFunc("PUT /api/v1/budgets/{month}", a.protected(true, a.putBudgets))
	mux.HandleFunc("POST /api/v1/transactions/{id}/complete", a.protected(true, a.completeSettlement))
	mux.HandleFunc("POST /api/v1/transactions/{id}/reopen", a.protected(true, a.reopenSettlement))
